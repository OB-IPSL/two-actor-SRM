...
#######################################################################################
          ...
          for ckpm in (1.,self.facteur,1./self.facteur):
            for ckim in (1.,self.facteur,1./self.facteur):
               ...
               if islink("plots"):
                 ...
                 print("i = ",i," point 2")
               if islink(self.listedir[i]):
                 os.unlink(self.listedir[i])
               elif isdir(self.listedir[i]):
                 shutil.rmtree(self.listedir[i],ignore_errors=True)
               os.mkdir(self.listedir[i])
               os.symlink(self.listedir[i],"plots")
               commande="./main.py --exp {:} --cki {:5.1f} --ckp {:5.1f} --ckim {:5.1f} --ckpm {:5.1f} --type-plot {:d} --load-noise {:} --app-title ".format(exp,
                                                                                                                                                              ckit,
                                                                                                                                                              ckpt,
                                                                                                                                                              ckim,
                                                                                                                                                              ckpm,
               ...
               os.unlink("{:}/scenario{:}.png".format(self.listedir[i],exp))
               os.unlink("{:}/test{:}.png".format(self.listedir[i],exp))
    ...
    if self.typeplot>=2:
      ...
      for i in range(1,nmodifs+1):
        imc.append(concatimages([im[0],im[i]],typeplot=self.typeplot))
