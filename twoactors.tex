...
\begin{equation} 
  e_{j}=-K^p(x_l-x_l^c) - K^d\frac{d(x_l-x^c_l)}{dt} - K^i\int_0^{t} \left(x_l(u)-x^c_l\right)du
\end{equation}
...
\begin{equation} 
  e_{j}=-\sum_{l=1}^{n_s}\alpha_l \left[K_{jl}^p(x_m-x_l^c) + K_{jl}^d\frac{d(x_l-x^c_l)}{dt} + K^i_{jl}\int_0^{t} (x_l(u)-x^c_l)du \right]
\end{equation}
