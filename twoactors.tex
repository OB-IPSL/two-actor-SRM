...
\begin{equation} 
  e_{j}()=-K^p_{jl}(x_l(t)-x_l^c) - K^d_{jl}\frac{d(x_l(t)-x^c_l)}{dt} - K^i_{jl}\int_0^{t} \left[x_l(u)-x^c_l\right]du
\end{equation}

\subsection{Correcteur PID multiple}

Ce correcteur détermine le vecteur $e$ à partir du vecteur d'état $x$ à l'instant $t$, 
de ses valeurs dans le passé\ $x(u)\  < t$, et de sa valeur cible $x^c$. Contrairement au cas précédent,
...
\begin{equation} 
  e_{j}=-\sum_{l=1}^{n_s}\alpha_l \left[K_{jl}^p(x_l(t)-x_l^c) + K_{jl}^d\frac{d(x_l(t)-x^c_l)}{dt} + K^i_{jl}\int_0^{t} \left[x_l(u)-x^c_l\right]du \right]
\end{equation}
...
Afin de faire varier le poids des différents temps passés dans le terme intégral $K_i \ldots$, on peut utiliser une fonction de poids $\beta$:
...
Cela permet 
\begin{itemize}
  \item soit de limiter la mémoire de la variable $x$ ($\beta(u)=0$\ pour\  $u\le t_1$ ou $u\le t-\Delta u$)
  \item soit de considérer un retard à l'action ($\beta(u)=0$\ si\ $u\ge t-\Delta u$)
\end{itemize}
\end{document}
